...
class Picker(ttk.Frame):
    ...
    def __init__(self, master=None,activebackground='#b1dcfb',values=[],entry_wid=None,activeforeground='black', selectbackground='#003eff', selectforeground='white', command=None, borderwidth=1, relief="solid",frameheight=120):
...
class Combopicker(ttk.Entry, Picker):
    def __init__(self, master, values= [] ,entryvar=None, entrywidth=None, entrystyle=None, onselect=None,activebackground='#b1dcfb', activeforeground='black', selectbackground='#003eff', selectforeground='white', borderwidth=1, relief="solid",frameheight=50):
        self.values=values
        self.master=master
        ...
        self.picker_frame = Picker(self.winfo_toplevel(), values= self.values,entry_wid = self.entry_var,activebackground=activebackground, 
                                   activeforeground=activeforeground, selectbackground=selectbackground, selectforeground=selectforeground, command=self._on_selected_check, frameheight=self.frameheight)
    ...
    def show_picker(self):
        ...
        if not self._is_menuoptions_visible:
            self.picker_frame = Picker(self.winfo_toplevel(), values=self.values,entry_wid = self.entry_var, frameheight=self.frameheight, activebackground=self.activebackground,
...
def asktime_plus(first_ask):
    ...
    if gencf.has_section("General")==False:
...
def askinfo(error=False,muti_ask=False,first_ask=False):
    ...
    def refresh_whitelist_entry(folder_names=[]):
        try:
            ...
            entry_keep_files = Combopicker(master=entryframe_keep_files, values = ['全选'] +folder_names +file_names, frameheight=120)
            entry_keep_files.grid(row=2, column=1,ipadx=240,pady=5,sticky='W')
            entry_keep_formats = Combopicker(master=entryframe_keep_formats, values = ['全选'] +exist_ends, frameheight=90)
    ...
    entry_old_path = ttk.Entry(root, textvariable = old_path, entry_wid=None)
    entry_old_path.grid(row=0, column=1, padx=10, pady=5,ipadx=190,sticky='W')
    ttk.Button(root, text= "浏览",command=lambda:selectPath(place='old',ori_content=entry_old_path.get())).grid(row=0,column=1,ipadx=3,sticky='E',padx=10)
    
    label_new_path = ttk.Label(root, text='新文件夹路径：')
    label_new_path.grid(row=1, column=0,pady=5,sticky='E')
    entry_new_path = ttk.Entry(root, textvariable = new_path, entry_wid=None)
    entry_new_path.grid(row=1, column=1, padx=10, pady=5,ipadx=190,sticky='W')
    ttk.Button(root, text= "浏览",command=lambda:selectPath(place='new',ori_content=entry_new_path.get())).grid(row=1,column=1,ipadx=3,sticky='E',padx=10)    
    ...
    entry_foldermove = ttk.Checkbutton(root, text='移动项目包括文件夹', variable=is_foldermove, command=lambda:movefolder(is_foldermove.get()))
    ...
    entry_time = ttk.Entry(root,entry_wid=None)
    ...
    def savefile(file=None):
        if not is_num():
            ...
            tkinter.messagebox.showwarning(title = 'Movefile',message='警告：请填写所有除白名单与开机自启勾选栏外的必填项目！')
        elif path_error():
