...
def get_available_versions(repo: GitHubRepo, process_tag: Callable[[str], Version] = None):
    ...
    logging.info(f"Searching for releases in 'http://github.com/{repo.user}/{repo.repo}/'...")
    request_url = f"http://api.github.com/repos/{repo.user}/{repo.repo}/releases"
    page_size = 100
    for i in itertools.count(1):
        data = json.loads(requests.get(request_url, dict(page=i, per_page=page_size), headers=AuthSession.header, allow_redirects=False, verify=False, timeout=5).text)
...
def get_latest_version(repo: GitHubRepo, process_tag: Callable[[str], Version] = None):
    if process_tag is None:
        process_tag = parse_tag
    logging.info(f"Searching for latest release in 'http://github.com/{repo.user}/{repo.repo}/'...")
    request_url = f"http://api.github.com/repos/{repo.user}/{repo.repo}/releases/latest"
    data = json.loads(requests.get(request_url, headers=AuthSession.header, allow_redirects=False, verify=False, timeout=5).text)
    if 'message' in data:
        return
...
def get_assets(repo: GitHubRepo, tag_name: str, assets_mask=re.compile('.*')):
    ...
    request_url = f"http://api.github.com/repos/{repo.user}/{repo.repo}/releases/tags/{tag_name}"
    data = json.loads(requests.get(request_url, headers=AuthSession.header, allow_redirects=False, verify=False, timeout=5).text)
    if 'message' in data:
        return []
